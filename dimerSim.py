...
class Experiment:
	...
	ATOM_MONOMER, ATOM_DIMER1, ATOM_DIMER2, ATOM_SITE, ATOM_REP = 0, 1, 2, 3, 4
	MOL_MONOMER, MOL_DIMER1, MOL_DIMER2 = 0, 1, 2
	
	# Monomer Temp
	moleculeTemp1 = map(Atom.fromTuple, [\
		(ATOM_MONOMER,0,0), \
		(ATOM_REP,0,1),(ATOM_REP,0,-1),(ATOM_REP,1,0),(ATOM_REP,-1,0), \
		(ATOM_REP,1,1),(ATOM_REP,-1,1),(ATOM_REP,1,-1),(ATOM_REP,-1,-1)])
	# I Dimer
	...
	def __init__(\
		...
		self.lat.setAtomTypeNum(5)
		self.lat.setEnergyMatrix(array([\
			[ 5,  3,  3, -5,  1],\
			[ 3,  3,  3,  0,  1],\
			[ 3,  3,  3,  0,  1],\
			[-5,  0,  0,  0,  0],\
			[ 1,  1,  1,  0,  0]]) * self.energyScale)

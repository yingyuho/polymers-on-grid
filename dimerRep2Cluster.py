...
#		initY = zeros(mNum)
	...
	def run(self, simulationTime, outputPeriod, display, outputFileName):
		...
		def doMonomerBirth(self):
			...
			return True
		def doMonomerBirth2A(self):
			newX = randint(floor(lat.dimX*0.2), floor(lat.dimX*0.4))
			newY = randint(floor(lat.dimY*0.2), floor(lat.dimY*0.4))
			lat.addMolecule(Molecule(MOL_MONOMER, newX, newY, moleculeTemp1))
			return True
		def doMonomerBirth2B(self):
			newX = randint(floor(lat.dimX*0.6), floor(lat.dimX*0.8))
			newY = randint(floor(lat.dimY*0.6), floor(lat.dimY*0.8))
			lat.addMolecule(Molecule(MOL_MONOMER, newX, newY, moleculeTemp1))
			return True
		...
		mvList = [ \
			...
			Movement(doMonomerBirth2A,expRandTime,lambda:0.5*self.monomerAppear), \
			Movement(doMonomerBirth2B,expRandTime,lambda:0.5*self.monomerAppear), \
